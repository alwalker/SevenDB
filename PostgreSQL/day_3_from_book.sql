...
SELECT TITLE FROM movies WHERE title ILIKE 'stardust%'; -- ILIKE = case insensitive 

SELECT TITLE FROM movies WHERE title ILIKE 'stardust_%'; --not at the end

SELECT COUNT(*) FROM movies WHERE title !~* '^the.*'; --regex ! = not match, ~ = regex, * = case insensitive
...
SELECT * FROM movies WHERE title % 'Avatre'; -- '% = trigram'

SELECT title FROM movies WHERE title @@ 'night & day'; -- @@ = full text
